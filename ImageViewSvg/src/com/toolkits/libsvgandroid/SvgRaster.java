...
public class SvgRaster {
	...
	public native static void RasterToBitmap(Bitmap bitmap, String svg);

	public static void debugMatrix(Matrix m) {
		Log.v("Kamoflage", m.toString());
	}

	public static void setTypeface(Paint p, String family, int weight_n_slant, float textSize, int talign) {
	...
	public static Shader createLinearGradient(float x1, float y1, float x2, float y2, int[] colors, float[] offsets, int spreadType) {
	...
	public static Shader createRadialGradient(float x, float y, float r, int[] colors, float[] offsets, int spreadType) {
		Shader.TileMode tm = Shader.TileMode.REPEAT;
		switch (spreadType) {
		case 0:
			tm = Shader.TileMode.REPEAT;
	...
	public static Matrix matrixInit(float xx, float yx, float xy, float yy, float x0, float y0) {
		...
		float[] val = { (float) xx, (float) xy, (float) x0, (float) yx, (float) yy, (float) y0, (float) 0.0, (float) 0.0, (float) 1.0 };
		x.setValues(val);
		return x;
	}
